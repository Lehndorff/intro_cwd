...
pred <- predict(fit_gbm, newdata=test,"prob")
...
pred <- predict(fit_gbm, newdata=test_2023,"prob")
...
prediction_output<-tourney_data %>% 
  filter(tourn_year==2023) %>% 
  cbind(pred) %>% 
  select(team, Seed, actual=rank, `0`:`6`) %>% 
  pivot_longer(-c(team, Seed, actual)) %>% 
  mutate(name=2^as.numeric(name)) %>% 
  pivot_wider()
